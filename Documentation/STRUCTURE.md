...
|---|---|---|
| `schema.sql` | Định nghĩa bảng SQLite | ✅ có |
| `corpus/` | `.xlsx`, `.json`, `.md` — dữ liệu thủ tục gốc | ✅ có |
| `runtime/app.db` | CSDL đang chạy: tài khoản, lịch sử, tệp tải lên | ❌ **không bao giờ** |

> **Quan trọng:** ứng dụng khi chạy **không đọc** `corpus/`. Câu trả lời lấy từ
> web qua MCP. `corpus/` chỉ phục vụ `Evaluation/` và fine-tune.
> *The running app does not read `corpus/` — answers come from live web search.*
...
Không chỗ nào khác trong mã nguồn mở CSDL trực tiếp, nên không phải sửa thêm.
...
mới) · `FAILURE_HANDLING.md` (hỏng thì làm gì) · `legacy/` (tài liệu V10.2 giữ
nguyên để đối chiếu).
